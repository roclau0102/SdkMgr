...
public class SdkMgr : EditorWindow
{
    ...
    /// 登录ip
    /// </summary>
    private static string loginIP = "119.23.106.59";
    /// <summary>
    /// 登录端口
    /// </summary>
    private static string loginPort = "8009";
    /// <summary>
    /// 应用id
    /// </summary>
    private static string bundleIdentifier = "com.antiphon.youlesangong";
    /// <summary>
    /// 版本号
    /// </summary>
    private static string serverVersion = "2017.11.10.1040";
    /// <summary>
    /// 编译目标
    /// </summary>
    ...
    private static void CopyWeiXinAndroid()
    {
    ...
    private static void CopyWeiXinIOS()
    {
    ...
    void OnGUI()
    {
        ...
        if (GUILayout.Button("设置", GUILayout.Width(50)))
        {
            ...
            {
                url += string.Format("client/apk/{0}-微信登录端{1}.apk", PlayerSettings.productName, serverVersion);
            }
            else if (buildTarget == BuildTarget.Android.GetHashCode() && plat == SDKPlatform.LOCAL)
            {
                url += string.Format("client/apk/{0}-客户端{1}.apk", PlayerSettings.productName, serverVersion);
            }
            else if (buildTarget == BuildTarget.iOS.GetHashCode())
            {
                url += string.Format("{0}-XCode", PlayerSettings.productName);
            }
    ...
    private static void GetConfig(int platform)
    {
    ...
    /// </summary>
    /// <param name="platform"></param>
    private static void SetConfig(int platform)
    {
